 ...
 * @returns true 表示语法合法。
 */
export function isCredentialRefName(ref) {
 ...
 */
export function findOpenRouterRoute(providers) {
    ...
    const entries = Object.entries(providers)
        ...
        const models = Array.isArray(record.models)
            ? record.models.filter((entry) => entry !== null && typeof entry === 'object' && typeof entry.id === 'string')
                .map((entry) => entry)
            : [];
 ...
 * @returns 路由与凭据快照。
 */
export async function readRouteState(settings, credentials) {
 ...
 */
export async function applyModelConfig(deps, model, options = {}) {
