 ...
 */
import { checkOpenRouterToken, fetchFreeModels } from "./openrouter.js";
import { applyModelConfig, readRouteState } from "./model-config.js";
...
export async function runOp(deps, request) {
    ...
    try {
        switch (request.op) {
            ...
            default:
                return { ok: false, code: 'unknown-op', error: `unknown op: ${JSON.stringify(request.op)}` };
        }
    }
    catch (error) {
...
async function runUseModel(deps, request) {
    ...
    try {
        const result = await applyModelConfig({ settings: deps.settings, credentials: deps.credentials }, model, { token: request.token, setDefault: request.setDefault === true });
        return {
            ok: true,
            modelId: model.id,
