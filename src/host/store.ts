 ...
 */
export async function loadStore(dir: string): Promise<PluginStore | null> {
  ...
  try {
    const parsed = JSON.parse(raw) as { version?: unknown; models?: unknown; fetchedAt?: unknown }
    if (!parsed || typeof parsed !== 'object' || !Array.isArray(parsed.models)) return null
    const models = (parsed.models as unknown[]).filter((entry): entry is FreeModel =>
      entry !== null && typeof entry === 'object' && typeof (entry as FreeModel).id === 'string')
    return {
...
function doSave(dir: string, store: PluginStore): Promise<void> {
  return (async () => {
 ...
 * @returns 写入完成后兑现。
 */
export function saveStore(dir: string, store: PluginStore): Promise<void> {
