/**
 * dsh-free-model —— tsdown 构建配置（对齐 dsh-jenkins / @lemcae/dsh-balance 的方案）。
 *
 * 两个产物：
 ...
 *   factory 的 require 解析宿主模块表（seed），因此 seed 表内的包保持 external，
 *   其余依赖全部内联。
 *
...
const clientConfig: UserConfig = {
  name: 'dsh-free-model/client',
  entry: { client: 'src/client/index.ts' },
  // 浏览器 bundle 与 node 半边同目录（lib/）；entry key 固定输出 lib/client.js。
  outDir: 'lib',
  format: 'cjs',
  platform: 'browser',
  target: 'es2024',
  ...
  sourcemap: true,
  clean: false,
  // type: module 与 cjs 默认输出 .cjs，这里统一为 .js（与 exports 的 ./lib/client.js 一致）。
  fixedExtension: false,
  outExtensions: () => ({ js: '.js' }),
  deps: {
  ...
  // __ModuleLoader__ 工厂包装（与 dsh-balance / dsh-jenkins 相同的方案）：
  // 产物整体位于 factory 函数体内，顶部 require(...) 解析到 factory 参数（seed 表）。
  banner: `window.__ModuleLoader__.load({ id: 'dsh-free-model', factory: (require) => {
